...
if [ "$1" = "config" ]; then
 echo 'graph_title Start-stop count'
# echo 'graph_args -l 0'
 echo 'graph_category disk'
 echo 'graph_scale no'
 echo 'graph_info This graph shows disk start-stop count.'
...
#   echo "${i}.min 0"
#   echo "${i}.max 60"
   echo "${i}.warning 1095"  # 5 years of run
   echo "${i}.critical 3650" # 7 years of run
  fi
 done
 exit 0
fi
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  ...
  LASTSTATE=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Start_Stop_Count/{print $0}' | /usr/bin/awk '{print $10}'`
