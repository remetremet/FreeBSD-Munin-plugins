...
if [ "$1" = "config" ]; then
        echo 'graph_title HDD errors & failures'
#        echo 'graph_args -l 0'
        echo 'graph_category disk'
        echo 'graph_scale no'
        echo 'graph_info This graph shows disk errors on the machine.'
 for i in $(/sbin/sysctl -n kern.disks)
 do
  if [[ -e "/dev/${i}${DISKID}" ]] ; then
   if [[ -e "${TEMPFILE}_${i}.info" ]]; then
    fts=`stat -f %m ${TEMPFILE}_${i}.info`
    fts=$((${fts}+3600))        # 1 hour old file
    if (( ${fts} > ${tts} )); then
     use=1
    else
     fts=$((${fts}+86400))        # 24 hours old file
     if (( ${fts} > ${tts} )); then
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  if [[ -e "${TEMPFILE}_${i}.all" ]]; then
   ...
   fts=$((${fts}+180))        # 3 minutes old file
   if (( ${fts} > ${tts} )); then
    use=1
   else
    fts=$((${fts}+28800))        # 8 hours old file
    if (( ${fts} > ${tts} )); then
     spinning=`/usr/local/sbin/smartctl -i -n never /dev/${i} | grep "Power mode" | awk '{ print $4 }'`
     if [ "${spinning}" = "ACTIVE" ]; then
      use=0
  ...
  Err1=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Reallocated_Sector_Ct/{print $0}' | /usr/bin/awk '{print $10}'`
  Err2=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Current_Pending_Sector/{print $0}' | /usr/bin/awk '{print $10}'`
  Err3=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Offline_Uncorrectable/{print $0}' | /usr/bin/awk '{print $10}'`
  LASTSTATE=`expr ${Err1:-0} + ${Err2:-0} + ${Err3:-0}`
