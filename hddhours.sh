...
if [ "$1" = "config" ]; then
 echo 'graph_title HDD power-on hours'
# echo 'graph_args -l 0'
 echo 'graph_category disk'
 echo 'graph_scale no'
 echo 'graph_info This graph shows disk power-on hours.'
...
#   echo "${i}.min 0"
#   echo "${i}.max 60"
   echo "${i}.warning 43830"  # 5 years of run
   echo "${i}.critical 61362" # 7 years of run
  fi
 done
 exit 0
fi
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  ...
  LASTSTATE=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Power_On_Hours/{print $0}' | /usr/bin/awk '{print $10}'`
