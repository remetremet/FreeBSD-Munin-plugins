...
# echo 'graph_args -l 0'
 ...
 echo 'graph_scale no'
 echo 'graph_info This graph shows disk temperature on the machine.'
...
#   echo "${i}.min 0"
#   echo "${i}.max 60"
   echo "${i}.warning 50"  # 5 years of run
   echo "${i}.critical 65" # 7 years of run
  fi
 done
 exit 0
fi
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  ...
  LASTSTATE=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/194 Temperature_Celsius/{print $0}' | /usr/bin/awk '{print $10}'`
