...
if [ "$1" = "config" ]; then
 echo 'graph_title Load cycle count'
# echo 'graph_args -l 0'
 echo 'graph_category disk'
 echo 'graph_scale no'
 echo 'graph_info This graph shows disk load cycle count.'
...
#   echo "${i}.min 0"
#   echo "${i}.max 60"
   echo "${i}.warning 35000"
   echo "${i}.critical 50000"
  fi
 done
 exit 0
fi
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  ...
  LASTSTATE=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Load_Cycle_Count/{print $0}' | /usr/bin/awk '{print $10}'`
