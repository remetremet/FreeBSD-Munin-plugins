...
if [ "$1" = "config" ]; then
 echo 'graph_title HDD power cycle count'
# echo 'graph_args -l 0'
 echo 'graph_category disk'
 echo 'graph_scale no'
 echo 'graph_info This graph shows disk power cycle count.'
...
#   echo "${i}.min 0"
#   echo "${i}.max 60"
   echo "${i}.warning 1095"  # once a day for 3 years
   echo "${i}.critical 3650" # twice a day for 5 years
  fi
 done
 exit 0
fi
...
do
 if [[ -e "/dev/${i}${DISKID}" ]] ; then
  ...
  LASTSTATE=`/bin/cat ${TEMPFILE}_${i}.all | /usr/bin/awk '/Power_Cycle_Count/{print $0}' | /usr/bin/awk '{print $10}'`
